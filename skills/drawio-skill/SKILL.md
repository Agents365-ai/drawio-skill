...
**macOS sandbox note (codex.app):** In some codex.app sandboxed environments on macOS, invoking the draw.io desktop CLI (even `draw.io --version`) can cause the draw.io process to crash and produce no CLI output. If that happens, treat the CLI as unavailable in this environment and use the fallback chain (browser URL / XML-only), or ask the user to run the export commands from a non-sandboxed host Terminal session and provide the outputs.

If the user reports that running the same command works in an “elevated” context, phrase the remediation as: **run the draw.io CLI outside the sandbox from a normal host Terminal session, ideally as a user account with sufficient permissions (e.g., an admin account),** rather than attempting to force CLI execution inside the sandbox.
...
**If `draw.io --version` crashes or prints nothing (common in restricted macOS sandboxes like codex.app):**
- Do not keep retrying CLI invocations inside the sandbox.
- Skip steps 4/7 (CLI export) and use **Browser fallback** (`scripts/encode_drawio_url.py`) or deliver the `.drawio` XML only.
- If the user needs PNG/SVG/PDF outputs, ask them to run the export commands in a **non-sandboxed host environment** and share the resulting files. If they have multiple user contexts available, prefer a host Terminal session under a user account with sufficient permissions (e.g., an admin account).
